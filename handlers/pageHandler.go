...
func GetPageGetHandler(c *fiber.Ctx) error {
...
func GetPagePostHandler(c *fiber.Ctx) error {
	payload := new(model.PagePayload)
	...
	if pageEntity, err := database.EntClient.Page.Query().Where(page.PathEQ(path)).Only(context.Background()); err != nil {
		...
		if payload.ReturnContent == "true" {
			content, _ := url.PathUnescape(pageEntity.Content)
...
func EditPageGetHandler(c *fiber.Ctx) error {
	accessToken := c.Query("access_token")
	title := c.Query("title")
	path, _ := url.PathUnescape(c.Params("path"))
	content := c.Query("content")
...
func EditPagePostHandler(c *fiber.Ctx) error {
	payload := new(model.PagePayload)
	...
	if _, err := database.EntClient.Account.Query().Where(account.AccessTokenEQ(payload.AccessToken)).Only(context.Background()); err == nil {
